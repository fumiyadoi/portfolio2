...
export default {
  ...
  async mounted () {
    ...
    // const q1 = await query(collection(db, 'books'), where('admin', '==', false), where('createdUserDocumentID', '==', self.$store.state.data.userId))
    const q1 = await query(collection(db, 'books'), where('createdUserDocumentID', '==', self.$store.state.data.userId))
    // const q1 = await query(collection(db, 'books'), where('admin', '==', false))
    const querySnapshot1 = await getDocs(q1)
    console.log(self.$store.state.data.userId)
    ...
    /* 共通コンテンツを取ってきます */
    const q2 = await query(collection(db, 'books'), where('admin', '==', true))
  ...
  methods: {
    ...
    goBookList () {
      ...
      this.$store.commit('data/changeIndetifier', 'sokutore-top')
    },
    startSokudoku () {
